...
public class AdvPushServiceImpl {
  ...
  public List<AdvDetail> getAdvDetailLimit(AdvVO vo) {
    Calendar cal = Calendar.getInstance();
    cal.set(Calendar.HOUR_OF_DAY, 0);
    cal.set(Calendar.SECOND, 0);
    cal.set(Calendar.MINUTE, 0);
    cal.set(Calendar.MILLISECOND, 0);
    vo.setTimestamp(String.valueOf(cal.getTimeInMillis()));
    return advPushMapper.getAdvDetailSize(vo);
  }

  public Long getAdvDetailCount(AdvVO vo) {
    return advPushMapper.getAdvDetailCount(vo);
  }
}
